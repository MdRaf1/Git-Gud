"""
Integration tests for Git Gud end-to-end workflows.
...
from git_gud.cli import app


class TestSafeCommandWorkflows:
...
class TestDangerousCommandWorkflows:
    ...
    @patch('git_gud.cli.execute_git_command')
    def test_force_push_confirmed_workflow(self, mock_execute, mock_confirm, mock_git_available):
    ...
    @patch('git_gud.cli.get_user_confirmation')
    @patch('git_gud.cli.execute_git_command')
    def test_reset_hard_confirmed_workflow(self, mock_execute, mock_confirm, mock_git_available):
    ...
    @patch('git_gud.cli.get_user_confirmation')
    @patch('git_gud.cli.execute_git_command')
    def test_clean_force_confirmed_workflow(self, mock_execute, mock_confirm, mock_git_available):
...
class TestErrorHandlingWorkflows:
    ...
    def test_git_not_available_workflow(self):
        ...
        with patch('git_gud.cli.is_git_available', return_value=False):
            result = self.runner.invoke(app, ["--execute", "git status"])
...
class TestKeyboardInterruptWorkflows:
    ...
    @patch('git_gud.cli.is_git_available')
    @patch('git_gud.cli.get_user_confirmation')
    def test_keyboard_interrupt_during_confirmation_workflow(self, mock_confirm, mock_git_available):
...
class TestRealGitIntegration:
    ...
    @pytest.mark.skipif(not hasattr(subprocess, 'run'), reason="subprocess.run not available")
    def test_real_git_status_workflow(self):
        ...
        assert ("On branch" in result.stdout or "working tree clean" in result.stdout or 
...
class TestComplexWorkflows:
    ...
    def test_complex_safe_command_workflow(self, mock_execute, mock_git_available):
        ...
        result = self.runner.invoke(app, ["--execute", "git log --graph --pretty=format:'%h (%d) %an <%ae> %ad %n%n    %s%n%n%b' --date=rfc -n 1"])
    ...
    @patch('git_gud.cli.get_user_confirmation')
    @patch('git_gud.cli.execute_git_command')
    def test_multiple_dangerous_patterns_workflow(self, mock_execute, mock_confirm, mock_git_available):
        ...
        result = self.runner.invoke(app, ["--execute", "git reset --hard HEAD~1 && git push --force"])
    ...
    @patch('git_gud.cli.is_git_available')
    @patch('git_gud.cli.execute_git_command')
    def test_command_with_special_characters_workflow(self, mock_execute, mock_git_available):
        ...
        result = self.runner.invoke(app, ["--execute", "git log --grep='Fix:.*#[0-9]+' --oneline -n 1"])
...
class TestWorkflowPerformance:
    ...
    def test_rapid_successive_commands_workflow(self):
        ...
        with patch('git_gud.cli.is_git_available', return_value=True), \
             patch('git_gud.cli.execute_git_command') as mock_execute:
