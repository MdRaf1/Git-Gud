...
def execute_git_command(command: str, timeout: int = 30) -> GitResult:
    ...
    if not is_git_available():
        return GitResult(
            stdout="",
            stderr="Git is not installed or not available in PATH. Please install Git and ensure it's accessible from the command line.",
    ...
    if not command or not command.strip():
        return GitResult(
            stdout="",
            stderr="Empty command provided. Please specify a Git command to execute.",
            exit_code=1,
            command=command,
            success=False
        )
    ...
    try:
        ...
        if not cmd_parts:
            return GitResult(
                stdout="",
                stderr="Invalid Git command format. Command cannot be empty after removing 'git' prefix.",
                exit_code=1,
                command=command,
                success=False
            )
    ...
    try:
        ...
        return GitResult(
            stdout="",
            stderr=f"Command timed out after {timeout} seconds. The Git operation may be taking too long or may be waiting for input.",
        ...
        return GitResult(
            stdout="",
            stderr="Git executable not found. Please ensure Git is installed and available in your system PATH.",
            exit_code=127,  # Command not found exit code
            command=command,
            success=False
        )
        ...
        return GitResult(
            stdout="",
            stderr="Permission denied when trying to execute Git command. Check file permissions and repository access rights.",
        ...
        return GitResult(
            stdout="",
            stderr=f"System error occurred while executing Git command: {str(e)}",
            exit_code=1,
            command=command,
            success=False
        )
        ...
        return GitResult(
            stdout="",
            stderr=f"Unexpected error occurred while executing Git command: {str(e)}",
            exit_code=1,
            command=command,
            success=False
        )